...
def write_report(results: list[Result], meta: dict | None = None) -> None:
    ...
    n_docs = (meta or {}).get("documents")
    n_chunks = (meta or {}).get("chunks")
    docs_phrase = f"{n_docs} documents" if n_docs else "the ingested corpus"
    chunks_phrase = f"{n_chunks} chunks" if n_chunks else "a corpus this size"
    below_floor = sum(
        1
        for r in in_corpus
        if r.top_similarity_unfiltered is not None
        and r.top_similarity_unfiltered < settings.similarity_threshold
    )
...
Generated by `backend/scripts/benchmark.py` on {(meta or {}).get("run_date") or date.today().isoformat()} against the
...
from a store of {corpus_line(meta)}.
...
little work at this corpus size - {chunks_phrase} fits in memory trivially - so treat this
...
![Top retrieval similarity per query, against the {settings.similarity_threshold} floor](similarity.svg)

Each dot is one query, placed at the similarity of its best-matching chunk. The gap is
the whole argument. The cost is in the same picture: {below_floor} in-corpus queries
...
- The corpus is {docs_phrase}. Retrieval latency will grow with corpus size; the
...
def main() -> None:
    ...
    if args.report_only:
        ...
        # Runs before the metadata block was added saved a bare list.
        meta, rows = (raw.get("meta"), raw["results"]) if isinstance(raw, dict) else (None, raw)
        write_report([Result(**r) for r in rows], meta)
        return

    asyncio.run(main_async(args))


def corpus_line(meta: dict | None) -> str:
    """The corpus size sentence for the report header.

    Read from the database at run time rather than hardcoded: the store grows as
    blocked sources are added by hand, and a stale "42 documents" in an evaluated
    document is worse than no figure at all.
    """
    if meta and meta.get("documents"):
        return f"{meta['documents']} documents / {meta['chunks']} chunks"
    return "the ingested corpus"


async def corpus_size(db) -> dict[str, int]:
...
def write_chart(results: list[Result]) -> None:
    """Emit docs/similarity.svg - the retrieval separation plot.

    Plan §11 asks for one chart, and this is the one worth drawing: every query's
    best similarity against the floor that decides whether it is answered at all.
    It shows the safety margin and its cost in the same picture - out-of-corpus
    queries sit well left of the floor, and so do the few in-corpus ones the floor
    turns away.

    Hand-rolled SVG rather than matplotlib: one chart does not justify a plotting
    dependency in the service image, and a committed .svg renders on GitHub with no
    build step. Colours are defined for both GitHub themes, and the background is
    painted explicitly - a transparent chart with dark text vanishes in dark mode.
    """
    floor = float(settings.similarity_threshold)

    def tops(kind: str) -> list[float]:
        return sorted(
            r.top_similarity_unfiltered
            for r in results
            if r.kind == kind
            and not r.red_flag
            and r.top_similarity_unfiltered is not None
        )

    series = [
        ("In-corpus", tops("in_corpus"), "a"),
        ("Out-of-corpus", tops("out_of_corpus"), "b"),
    ]
    values = [v for _, vs, _ in series for v in vs]
    if not values:
        print("chart       -> skipped (no similarity data)")
        return
    ...
    step = 0.05
    lo = math.floor((min(values) - 0.025) / step) * step
    hi = math.ceil((max(values) + 0.025) / step) * step
    lo = min(lo, floor - step)
    hi = max(hi, floor + step)

    def xf(v: float) -> float:
        return ML + (v - lo) / (hi - lo) * PW

    def lay_out(vs: list[float]) -> list[tuple[float, int]]:
        """Beeswarm: push a point into the next free lane when it would overlap."""
        ...
        for v in vs:
            ...
            for li in range(len(lanes) + 1):
                if li == len(lanes):
                    ...
                    placed.append((x, li))
                    break
                if x - lanes[li] >= 2 * R + 1.5:
                    lanes[li] = x
                    placed.append((x, li))
                    break
        return placed

    def lane_dy(li: int) -> float:
        return 0.0 if li == 0 else (-1 if li % 2 else 1) * 12.0 * ((li + 1) // 2)

    p: list[str] = []
    p.append(
        f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" '
        f'width="{W}" height="{H}" role="img" '
        f'aria-label="Top retrieval similarity per query, against the {floor} floor">'
    )
    p.append(
        "<style>"
        ".bg{fill:#ffffff}.ttl{fill:#1f2328}.sub{fill:#656d76}.lbl{fill:#656d76}"
        ".ax{stroke:#d0d7de}.zone{fill:#cf222e;fill-opacity:.05}"
        ".flr{stroke:#cf222e;stroke-opacity:.85}.flrT{fill:#cf222e}"
        ".a{fill:#1f6feb}.b{fill:#bf8700}"
        "@media(prefers-color-scheme:dark){"
        ".bg{fill:#0d1117}.ttl{fill:#e6edf3}.sub{fill:#8b949e}.lbl{fill:#8b949e}"
        ".ax{stroke:#30363d}.zone{fill:#f85149;fill-opacity:.08}"
        ".flr{stroke:#f85149}.flrT{fill:#f85149}"
        ".a{fill:#58a6ff}.b{fill:#d29922}}"
        "text{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif}"
        "</style>"
    )
    p.append(f'<rect class="bg" x="0" y="0" width="{W}" height="{H}"/>')
    p.append(
        f'<text class="ttl" x="{ML}" y="30" font-size="15" font-weight="600">'
        "Top retrieval similarity per query</text>"
    )
    p.append(
        f'<text class="sub" x="{ML}" y="48" font-size="12">'
        f"Everything left of the {floor:g} floor is refused rather than answered"
        "</text>"
    )

    fx = xf(floor)
    p.append(
        f'<rect class="zone" x="{ML}" y="{MT + 20}" width="{fx - ML:.1f}" '
        f'height="{axis_y - MT - 20}"/>'
    )

    for i, (label, vs, cls) in enumerate(series):
        y = rows_y[i]
        p.append(
            f'<text class="lbl" x="{ML - 14}" y="{y + 4}" font-size="12.5" '
            f'text-anchor="end">{_svg_escape(label)}</text>'
        )
        p.append(
            f'<text class="lbl" x="{ML - 14}" y="{y + 20}" font-size="10.5" '
            f'text-anchor="end" opacity=".75">n={len(vs)}</text>'
        )
        for x, li in lay_out(vs):
            p.append(
                f'<circle class="{cls}" cx="{x:.1f}" cy="{y + lane_dy(li):.1f}" '
                f'r="{R}" fill-opacity=".62"/>'
            )

    p.append(
        f'<line class="flr" x1="{fx:.1f}" y1="{MT + 12}" x2="{fx:.1f}" y2="{axis_y}" '
        'stroke-width="1.5" stroke-dasharray="5 4"/>'
    )
    p.append(
        f'<text class="flrT" x="{fx:.1f}" y="{MT + 4}" font-size="11.5" '
        f'font-weight="600" text-anchor="middle">floor {floor:g}</text>'
    )

    p.append(f'<line class="ax" x1="{ML}" y1="{axis_y}" x2="{ML + PW}" y2="{axis_y}"/>')
    n_ticks = int(round((hi - lo) / step))
    for i in range(n_ticks + 1):
        v = lo + i * step
        x = xf(v)
        p.append(f'<line class="ax" x1="{x:.1f}" y1="{axis_y}" x2="{x:.1f}" y2="{axis_y + 5}"/>')
        p.append(
            f'<text class="lbl" x="{x:.1f}" y="{axis_y + 19}" font-size="11" '
            f'text-anchor="middle">{v:.2f}</text>'
        )
    p.append(
        f'<text class="lbl" x="{ML + PW / 2:.1f}" y="{H - 12}" font-size="11.5" '
        'text-anchor="middle">cosine similarity of the best-matching chunk</text>'
    )
    p.append("</svg>")

    path = DOCS / "similarity.svg"
